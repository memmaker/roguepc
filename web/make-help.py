...
<li><kbd>S</kbd> (then <kbd>Enter</kbd>) saves and ends the session, as in the original; reload the page (or press <em>Play again</em>) to continue.</li>
...
<li>Window layout, zoom, tiles/text and auto_more are stored in the same browser storage.</li>
<li>Private/incognito windows and "clear site data" delete the stored game. Export first if it matters.</li>
</ul>'''

WEB = '''<ul>
<li><strong>Tiles / Text</strong> in the top bar (or <kbd>F12</kbd>) switch between the tiled windows and the original IBM PC text screen: VGA font, CP437 symbols, CGA colours, one window.</li>
...
<li><strong>auto_more</strong> skips the --More-- prompts; everything stays in the Messages window.</li>
...
parts.append(section('about', 'About the game',
                     guide.pop('How Rogue PC differs from Angband')))
